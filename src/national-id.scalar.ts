...
@Scalar("NationalID")
export class NationalIDScalar {
  parseValue(value: any) {
    // Validate the national ID here
  ...
  serialize(value: any) {
    // Convert the national ID to a string for output
    return String(value);
  }

  parseLiteral(ast: ValueNode) {
    // Parse the AST node and validate the national ID
    if (ast.kind !== Kind.STRING) {
      throw new BadRequestException("Invalid national ID");
    }
    if (!isValidNationalID(ast.value)) {
      throw new BadRequestException("Invalid national ID");
    }
    return ast.value;
  }
...
const isValidNationalID = (code) => {
    ...
    return (
      (s < 2 && numbers[numbers.length - 1] === s) ||
